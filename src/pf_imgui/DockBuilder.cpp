...
void SubDockBuilder::run(ImGuiID &parentNodeId) {
  auto nodeId = ImGui::DockBuilderSplitNode(parentNodeId, static_cast<ImGuiDir>(direction), sizeRatio, nullptr, &parentNodeId);
  std::ranges::for_each(subCommands, [&](auto &val) {
    std::visit(
        Visitor{[&](const details::DockWindowCmd &cmd) { ImGui::DockBuilderDockWindow(cmd.window.getImGuiName().c_str(), nodeId); },
                [&](const std::unique_ptr<SubDockBuilder> &subDockBuilder) { subDockBuilder->run(nodeId); },
                [&](const details::DockSizeCmd &sizeCmd) { ImGui::DockBuilderSetNodeSize(nodeId, static_cast<ImVec2>(sizeCmd.size)); }},
        val);
  });
}
...
void DockBuilder::run() {
  ...
  std::ranges::for_each(subCommands, [&](auto &val) {
    std::visit(
        Visitor{
            [&](const details::DockWindowCmd &cmd) { ImGui::DockBuilderDockWindow(cmd.window.getImGuiName().c_str(), dockspaceId); },
            [&](const std::unique_ptr<SubDockBuilder> &subDockBuilder) { subDockBuilder->run(dockspaceId); },
            [&](const details::DockSizeCmd &sizeCmd) { ImGui::DockBuilderSetNodeSize(dockspaceId, static_cast<ImVec2>(sizeCmd.size)); }},
        val);
  });
}
