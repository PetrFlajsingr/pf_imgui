...
std::vector<Renderable *> AnchorLayout::getRenderables() {
  return children | ranges::views::transform([](auto &child) -> Renderable * { return child.element.get(); }) | ranges::to_vector;
}
...
void AnchorLayout::setChildPosition(const std::string &childName, Position position) {
  ...
  if (const auto iter = std::ranges::find_if(childrenAddr, [childName](auto child) { return child->element->getName() == childName; });
...
void AnchorLayout::removeChild(const std::string &childName) {
  if (auto iter = std::ranges::find_if(children, [childName](const auto &child) { return child.element->getName() == childName; });
      iter != children.end()) {
    children.erase(iter);
  }
}
