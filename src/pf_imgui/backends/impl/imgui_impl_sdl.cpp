...
static bool ImGui_ImplSDL2_Init(SDL_Window *window, void *sdl_gl_context) {
  ...
  if (SDL_GetWindowWMInfo(window, &info)) main_viewport->PlatformHandleRaw = info.info.win.window;
#endif
...
#endif
  if (!ImGui_ImplSDL2_Init(window, NULL)) return false;
  ImGui_ImplSDL2_Data *bd = ImGui_ImplSDL2_GetBackendData();
  bd->UseVulkan = true;
  return true;
}
...
bool ImGui_ImplSDL2_InitForSDLRenderer(SDL_Window *window) { return ImGui_ImplSDL2_Init(window, NULL); }
...
static void ImGui_ImplSDL2_UpdateMousePosAndButtons() {
  ...
  io.MouseDown[0] = bd->MousePressed[0]
  ...
  SDL_Window *hovered_window = SDL_HAS_MOUSE_FOCUS_CLICKTHROUGH
      ? SDL_GetMouseFocus()
      : NULL;// This is better but is only reliably useful with SDL 2.0.5+ and SDL_HINT_MOUSE_FOCUS_CLICKTHROUGH.
...
static void ImGui_ImplSDL2_CreateWindow(ImGuiViewport *viewport) {
  ...
  SDL_SysWMinfo info;
  SDL_VERSION(&info.version);
  if (SDL_GetWindowWMInfo(vd->Window, &info)) viewport->PlatformHandleRaw = info.info.win.window;
#endif
}
