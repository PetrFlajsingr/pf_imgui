...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData *draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline) {
  ...
  ImGuiViewportDataVulkan *viewport_renderer_data =
      (ImGuiViewportDataVulkan *) draw_data->OwnerViewport->RendererUserData;
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer) {
  ...
  VkDescriptorSet font_descriptor_set = (VkDescriptorSet) ImGui_ImplVulkan_AddTexture(
      g_FontSampler, g_FontView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo *info, VkRenderPass render_pass) {
  IM_ASSERT(g_FunctionsLoaded
            && "Need to call ImGui_ImplVulkan_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES "
               "are set!");
...
void ImGui_ImplVulkan_Shutdown() {
  ...
  ImGuiViewport *main_viewport = ImGui::GetMainViewport();
  if (ImGuiViewportDataVulkan *data = (ImGuiViewportDataVulkan *) main_viewport->RendererUserData) IM_DELETE(data);
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport *viewport, void *) {
  ...
  {
    {
      err = vkAcquireNextImageKHR(v->Device, wd->Swapchain, UINT64_MAX, fsd->ImageAcquiredSemaphore, VK_NULL_HANDLE,
                                  &wd->FrameIndex);
      check_vk_result(err);
