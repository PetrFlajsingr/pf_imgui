...
bool ImGui::LabeledDragScalar(const char *label, ImGuiDataType data_type, void *p_data, float v_speed, const void *p_min, const void *p_max,
                              const char *format, const char *componentLabel, ImU32 componentColor, ImGuiSliderFlags flags) {
  ...
  if (!temp_input_is_active) {
    ...
    const bool make_active =
        (input_requested_by_tabbing || clicked || double_clicked || g.NavActivateId == id || g.NavActivateInputId == id);
  ...
  RenderTextClipped(componentLabelBB.Min, componentLabelBB.Max, componentLabel, componentLabel + std::strlen(componentLabel), nullptr,
                    ImVec2(0.5f, 0.5f));
...
bool ImGui::LabeledDragScalarN(const char *label, ImGuiDataType data_type, void *p_data, int components, float v_speed, const void *p_min,
                               const void *p_max, const char *format, const char **componentLabels, const ImU32 *componentColors,
  ...
  for (int i = 0; i < components; i++) {
    ...
    value_changed |= LabeledDragScalar("", data_type, p_data, v_speed, p_min, p_max, format, componentLabels[i], componentColors[i], flags);
    PopID();
    PopItemWidth();
