...
std::optional<std::unique_ptr<Pin>> NodeEditorLoading::constructPin(PinTypeIdentifier typeId, ig::Node *parent,
                                                                    const toml::table &src) {
  if (auto iter = pinConstructors.find(typeId); iter != pinConstructors.end()) { return iter->second(parent, src); }
  return std::nullopt;
}
std::optional<std::unique_ptr<Node>> NodeEditorLoading::constructNode(NodeTypeIdentifier typeId, ig::NodeEditor *parent,
                                                                      const toml::table &src) {
