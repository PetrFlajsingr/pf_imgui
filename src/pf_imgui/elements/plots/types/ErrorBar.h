 ...
 protected:
  void renderImpl() override {
    if constexpr (Type == BarType::Vertical) {
      ImPlot::PlotErrorBars(getLabel().c_str(), xData.data(), yData.data(), error.data(),
                            xData.size());
    } else {
      ImPlot::PlotErrorBarsH(getLabel().c_str(), xData.data(), yData.data(), error.data(),
                             xData.size());
    }
  }
