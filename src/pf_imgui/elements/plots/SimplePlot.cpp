...
SimplePlot::SimplePlot(const std::string &elementName, std::unique_ptr<Resource<std::string>> label, PlotType plotType,
    ...
    : Element(elementName), Labellable(std::move(label)), Resizable(size), plotType(plotType), values(std::move(values)),
...
void SimplePlot::renderImpl() {
  ...
  switch (plotType) {
    case PlotType::Lines:
      ImGui::PlotLines(getLabel().get().c_str(), values.data(), static_cast<int>(values.size()), 0,
                       overlayText.has_value() ? overlayText->c_str() : nullptr, scaleMin, scaleMax,
                       getSize().asImVec());
      break;
    case PlotType::Histogram:
      ImGui::PlotHistogram(getLabel().get().c_str(), values.data(), static_cast<int>(values.size()), 0,
                           overlayText.has_value() ? overlayText->c_str() : nullptr, scaleMin, scaleMax,
                           getSize().asImVec());
      break;
  }
