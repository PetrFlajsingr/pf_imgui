...
concept CustomBoxItemFactory = std::is_invocable_r_v<std::unique_ptr<R>, F, T>;
 ...
 */
  CustomItemBox(const std::string &elementName, const std::string &label, CustomBoxItemFactory<T, R> auto &&rowFactory)
 ...
 */
   ...
   * @param rowFactory
   */
  void setItemFactory(CustomBoxItemFactory<T, R> auto &&rowFactory) {
