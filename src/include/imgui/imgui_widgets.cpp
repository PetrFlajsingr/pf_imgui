...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
  ...
  if (flags & ImGuiSeparatorFlags_Vertical)
  {
    ...
    // NOTE: fix for vertical separator size for Horizontal/Vertical PR
    float y2 = window->DC.CursorPos.y + window->DC.CurrLineSize.y + (window->DC.CurrentLayout ? window->DC.CurrentLayout->MinimumSize.y : 0);
...
void ImGui::Separator()
{
  ...
  if (window->SkipItems)
    return;
  // NOTE: modified for Vertical/Horizontal PR
  const bool isHorizontal = (window->DC.LayoutType == ImGuiLayoutType_Horizontal) || (window->DC.CurrentLayout && window->DC.CurrentLayout->Type == ImGuiLayoutType_Horizontal);
  // Those flags should eventually be overridable by the user
  ImGuiSeparatorFlags flags = isHorizontal ? ImGuiSeparatorFlags_Vertical : ImGuiSeparatorFlags_Horizontal;
