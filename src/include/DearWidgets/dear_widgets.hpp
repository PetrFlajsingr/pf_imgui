...
inline Type Normalize01(Type const x, Type const _min, Type const _max) {
...
template<typename Type>
inline Type Rescale(Type const x, Type const _min, Type const _max, Type const newMin, Type const newMax) {
  return ScaleFromNormalized(Normalize01(x, _min, _max), newMin, newMax);
}

inline float ImDot(ImVec2 const a, ImVec2 const b) { return a.x * b.x + a.y * b.y; }

inline float ImDot(ImVec4 const a, ImVec4 const b) { return a.x * b.x + a.y * b.y + a.z * b.z + a.w * b.w; }

inline float ImFract(float x) {
  float iPart;
  return std::modf(x, &iPart);
}

inline float ImSmoothStep(float edge0, float edge1, float x) {
...
inline float ImLength(ImVec2 v) { return ImSqrt(ImLengthSqr(v)); }

inline float ImLength(ImVec4 v) { return ImSqrt(ImLengthSqr(v)); }

inline float LinearSample(float t, float *buffer, int count) {
#if 0
...
inline float Roughness(float x0, float x1, float x2, float x3, float x4) {
  ...
  [[maybe_unused]] float const rms = (dx0 * dx0 + dx1 * dx1 + dx2 * dx2 + dx3 * dx3 + dx4 * dx4);

  [[maybe_unused]] float const ddx0 = ImAbs(x0 - x1);
  float const ddx1 = ImAbs(x1 - x2);
  float const ddx2 = ImAbs(x2 - x3);
  [[maybe_unused]] float const ddx3 = ImAbs(x3 - x4);
...
inline void DrawColorDensityPlotEx(ImDrawList *pDrawList, FuncType func, float minX, float maxX, float minY, float maxY,
    ...
    for (int j = 0; j < resolutionY; ++j) {
      ...
      if constexpr (IsBilinear) {
        ...
        pDrawList->AddRectFilledMultiColor(position + ImVec2(sx * (i + 0), sy * (j + 0)),
                                           position + ImVec2(sx * (i + 1), sy * (j + 1)), col00, col10, col11, col01);
      } else {
        pDrawList->AddRectFilledMultiColor(position + ImVec2(sx * (i + 0), sy * (j + 0)),
                                           position + ImVec2(sx * (i + 1), sy * (j + 1)), col00, col00, col00, col00);
      }
    }
  }
}
...
template<bool IsBilinear, typename FuncType>
inline void DrawColorBandEx(ImDrawList *pDrawList, ImVec2 const vpos, ImVec2 const size, FuncType func, int division,
                            float gamma) {
  ...
  auto curColor = [gamma, &func](float const x, float const) { return func(ImPow(x, gamma)); };

  DrawColorDensityPlotEx<IsBilinear>(pDrawList, curColor, 0.0f, 1.0f, 0.0f, 0.0f, vpos, size, division, 1);
}

template<bool IsBilinear, typename FuncType>
inline void DrawColorRingEx(ImDrawList *pDrawList, ImVec2 const curPos, ImVec2 const size, float thickness_,
                            FuncType func, int division, float colorOffset) {
...
inline void DrawChromaticPlotEx(ImDrawList *pDrawList, ImVec2 const vPos, float width, float heigth,
  ...
  if constexpr (IsBilinear)
    DrawChromaticPlotBilinear(pDrawList, vPos, width, heigth, chromeLineSamplesCount, colorspace, observer, illum, resX,
                              resY, maskColor, wavelengthMin, wavelengthMax, minX, maxX, minY, maxY);
  else
    DrawChromaticPlotNearest(pDrawList, vPos, width, heigth, chromeLineSamplesCount, colorspace, observer, illum, resX,
                             resY, maskColor, wavelengthMin, wavelengthMax, minX, maxX, minY, maxY);
}
...
inline void AnalyticalPlotEx(char const *label, FuncType func, float const minX, float const maxX,
  ...
  std::transform(roughness.begin(), roughness.end(), roughness.begin(), [minRough, maxRough](float const xx) {
    return ImPow(ImSaturate(1.0f - (xx - minRough) / (maxRough - minRough)), 1.0f);
  });
  ...
  if (pts.size() > ((1 << (sizeof(ImDrawIdx) * 4)) - 1)) pts.resize(((1 << (sizeof(ImDrawIdx) * 4)) - 1));
...
template<bool IsBilinear, typename FuncType>
inline bool DensityPlotEx(const char *label, FuncType func, int resX, int resY, float minX, float maxX, [[maybe_unused]]float minY,
                          float maxY) {
  ImGuiID const iID = ImGui::GetID(label);
  ImGui::PushID(iID);
  ...
  DrawColorDensityPlotEx<IsBilinear>(pDrawList, GetColor, minX, maxX, minX, maxY, curPos, ImVec2(width, height), resX,
                                     resY);
