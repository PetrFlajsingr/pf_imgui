...
public:
	...
	NOTIFY_INLINE auto get_elapsed_time() { return std::chrono::steady_clock::now() - this->creation_time; }
	...
	NOTIFY_INLINE auto get_fade_percent() -> float
	{
		...
		{
			return (1.f - (((float)elapsed.count() - (float)NOTIFY_FADE_IN_OUT_TIME - (float)this->dismiss_time) / (float)NOTIFY_FADE_IN_OUT_TIME)) * NOTIFY_OPACITY;
		}
