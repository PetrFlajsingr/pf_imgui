...
class ImGuiToast {
 ...
 public:
  ...
  NOTIFY_INLINE auto get_phase() -> ImGuiToastPhase {
    ...
    if (elapsed.count() > NOTIFY_FADE_IN_OUT_TIME + this->dismiss_time + NOTIFY_FADE_IN_OUT_TIME) {
      return ImGuiToastPhase_Expired;
    } else if (elapsed.count() > NOTIFY_FADE_IN_OUT_TIME + this->dismiss_time) {
      return ImGuiToastPhase_FadeOut;
    } else if (elapsed.count() > NOTIFY_FADE_IN_OUT_TIME) {
  ...
  NOTIFY_INLINE auto get_fade_percent() -> float {
    ...
    if (phase == ImGuiToastPhase_FadeIn) {
      return ((float) elapsed.count() / (float) NOTIFY_FADE_IN_OUT_TIME) * NOTIFY_OPACITY;
    } else if (phase == ImGuiToastPhase_FadeOut) {
      return (1.f
              - (((float) elapsed.count() - (float) NOTIFY_FADE_IN_OUT_TIME - (float) this->dismiss_time)
 ...
 public:
  ...
  ImGuiToast(ImGuiToastType type, int dismiss_time, const char *format, auto &&...args)
...
/// </summary>
NOTIFY_INLINE void InsertNotification(const ImGuiToast &toast, std::vector<ImGuiToast> &notifications) {
...
NOTIFY_INLINE void RemoveNotification(int index, std::vector<ImGuiToast> &notifications) {
...
NOTIFY_INLINE void RenderNotifications(std::vector<ImGuiToast> &notifications) {
  ...
  for (std::size_t i = 0; i < notifications.size(); i++) {
    ...
    SetNextWindowPos(ImVec2(vp_size.x - NOTIFY_PADDING_X, vp_size.y - NOTIFY_PADDING_Y - height), ImGuiCond_Always,
                     ImVec2(1.0f, 1.0f));
    if (current_toast->first) {
      current_toast->first = false;
      SetNextWindowFocus();
    }
    ...
    {
      PushTextWrapPos(
          vp_size.x / 3.f);// We want to support multi-line text, this will wrap the text after 1/3 of the screen width
